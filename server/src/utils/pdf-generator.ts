...
function addFooter(document: PdfDocument) {
  ...
  for (let index = range.start; index < range.start + range.count; index += 1) {
    ...
    document
      ...
      .fontSize(8)
      .fillColor(colors.muted)
      .text('Generated by WorkNavo', page.left, footerY, {
        lineBreak: false,
      })
...
function addInvoiceTableHeader(
  ...
  document
    .roundedRect(page.left, y, layout.tableWidth, 28, 7)
    .fill(colors.dark);
  document
    .font('Helvetica-Bold')
    .fontSize(8)
...
function addInvoiceItems(document: PdfDocument, invoice: InvoiceDocument) {
  ...
  invoice.items.forEach((item, index) => {
    ...
    document
      ...
      .font('Helvetica-Bold')
      .text(formatMoney(item.amount, invoice.currency), layout.amountX, y + 6, {
        align: 'right',
        width: layout.amountWidth,
      });
...
export function pdfFilename(value: string) {
  ...
  return `${safeValue || 'worknavo-document'}.pdf`;
}
